...
- Using `Agent` when `Ask` or `Plan` would have been safer.
- Not attaching relevant files or folders as context.
- Accepting plausible output without checking it.
- Repeating the same instructions in every chat instead of storing them in instruction files.
- Staying in one long conversation after the task has changed.

## Recommended Starter Prompts
- `Explain the difference between Ask, Plan, and Agent in VS Code for a beginner.`
- `Create a step-by-step plan for a beginner BI or data engineering lab about Copilot prompt quality.`
- `Review #file and identify which parts are unclear for first-time BI, DS, or DE learners.`
- `Using #codebase, find the best place to add a troubleshooting section for a data team onboarding module.`
- `Summarize this official page for a beginner audience: #fetch https://code.visualstudio.com/docs/copilot/agents/overview`
...
## References
See the companion reference file in `references/copilot-agent-beginner-best-practices.md`.
